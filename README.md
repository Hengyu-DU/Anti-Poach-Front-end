...
1. 确定首页主要外观，模仿对象：Altas of living Australia `www.ala.org.au`，基于此进行优化改进
2. 创建react项目
...
1. 放在服务器上后，初次访问页面会出现icon在js脚本加载后出现的情况。改boostrap icon 的 CDN 为npm依赖包，解决了bug。

2. 根据warning提示修改了一些代码细节
